...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
    ...
    m_nDefCellWidth      = 20;        //    when the window is created or subclassed
	...
	m_SelRowLen = 1000;
	m_SelRowList = new short int[m_SelRowLen]; 
	for (int i =0;i<m_SelRowLen;i++) *(m_SelRowList+i) = 0;
}
...
void CGridCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
    ...
    if ((next != m_idCurrentCell)&&m_CellMoveFlag) 
    {
...
void CGridCtrl::SelectRow(int Row)
{
	...
	for (j=1;j<N;j++)
	{
		nState = GetItemState(Row,j);
		SetItemState(Row, j, nState | GVIS_SELECTED);
	}
	*(m_SelRowList+Row) = 1;
	
}

void CGridCtrl::UnSelectRow(int Row)
{
	int j,N,nSel;
	UINT nState;
	//N = GetRowCount();
	//nSel=0;
	//for (j=1;j<N;j++)
	//if (*(m_SelRowList+j))nSel++;
	if ((!m_NoUnSelectRow)||(nSel<2))
	{
                                 ...
                                 BOOL bForceRepaint /* = FALSE */)
{
    ...
    for (pos = m_PrevSelectedCellMap.GetStartPosition(); pos != NULL; /* nothing */)
    {
        ...
        SetItemState(cell.row, cell.col, nState | GVIS_SELECTED);
			...
			*(m_SelRowList+cell.row) = 1;
		// copy selection information
		//if (nState & GVIS_SELECTED) *(m_SelRowList+cell.row) = 1;
		//else *(m_SelRowList+cell.row) = 0;
         ...
         nMinRow <= nMaxRow && nMinCol <= nMaxCol)
    {
        for (int row = nMinRow; row <= nMaxRow; row++)
            for (int col = nMinCol; col <= nMaxCol; col++) 
            {
				...
				//if (nState & GVIS_SELECTED) *(m_SelRowList+row) = 1;
				//else *(m_SelRowList+row) = 0;
...
void CGridCtrl::OnSelecting(const CCellID& currentCell)
{
    ...
    switch(m_MouseMode)
    {
        ...
        case MOUSE_SELECT_ROW:   SelectRows(currentCell);if ((!m_ViewNoSelect)&&(m_pView!=NULL)) m_pView->UpdateSelection(m_SelRowList,0);     break;
        case MOUSE_SELECT_CELLS: SelectCells(currentCell);if ((!m_ViewNoSelect)&&(m_pView!=NULL)) m_pView->UpdateSelection(m_SelRowList,m_ViewReDraw);break;
    }
}
...
void CGridCtrl::ResetScrollBars()
{
    ...
    if (VisibleRect.Height() < VirtualRect.Height())
        ...
        //m_nVScrollMax = 0;
		m_nVScrollMax = VisibleRect.Height()-1;
    ...
    SetScrollRange(SB_VERT, 0, m_nVScrollMax+1, TRUE);
    if (m_HScrollFlag)
...
BOOL CGridCtrl::SetRowCount(int nRows)
{
    ...
    if (nRows > m_SelRowLen-1)
	{
		newList = new short int[nRows+1];
		for (i=0;i<m_SelRowLen;i++)
			*(newList+i) = *(m_SelRowList+i);
		for (i=m_SelRowLen;i<nRows+1;i++)
			*(newList+i) = 0;
		delete m_SelRowList;
		m_SelRowLen = nRows+1;
		m_SelRowList = newList;
	}
	m_nRows = nRows;
...
void CGridCtrl::OnMouseMove(UINT nFlags, CPoint point)
{
    CRect rect;
    GetClientRect(rect);
	if (m_LButtonDown) m_ViewReDraw = 0;
#ifndef GRIDCONTROL_NO_DRAGDROP
...
void CGridCtrl::OnRButtonDown(UINT nFlags, CPoint point) 
{
	...
	if (!cell.row)
	{
		float X =0; float Mean = 0; float SD = 0;
		int Count =0;
		int N = GetRowCount();
		for (int j=2; j<N-2; j++)
			if(*(m_SelRowList+j)) 
			{
				X=atof(GetItemText(j, cell.col));
				Mean+=X;
				SD+=X*X;
				Count++;
			}
		...
		{
			Mean/=Count;
			SD = sqrt(fabs(SD/Count-Mean*Mean));
			sprintf(szNum,"%4.3f",Mean);
			SetItemText(N-2,cell.col, szNum);
			sprintf(szNum,"%4.3f",SD);
			SetItemText(N-1,cell.col, szNum);
			RedrawRow(N-2);
			RedrawRow(N-1);
			//SetFocusCell(N-2,cell.col);
	...
	*/
	else
	{
		...
		if (!cell.col)
		{
			...
			if ((!m_ViewNoSelect)&&(m_pView!=NULL)) 
				m_pView->UpdateSelection(m_SelRowList,m_ViewReDraw);
			RedrawRow(cell.row);
		}
...
void CGridCtrl::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	...
	m_CellMoveFlag=1;
	CCellID cell = GetCellFromPt(point);
	// View Update selection is forbiden when entire column celected
...
void CGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	int row = m_LeftClickDownCell.row;
	int col = m_LeftClickDownCell.col;
    if (!IsValid(m_LeftClickDownCell)) return;
	if ((!m_LeftClickDownCell.row)&&m_LeftClickDownCell.col)
		m_ViewNoSelect =1;
	else m_ViewNoSelect =0;
...
{	
	m_ViewReDraw = 1;
	m_CellMoveFlag=1;
	if (m_LButtonDown&&(!m_ViewNoSelect)) 
		if (m_pView!=NULL)
			m_pView->UpdateSelection(m_SelRowList, m_ViewReDraw);
			//m_pView->UpdateSelection(m_SelRowList, 0);
	m_LButtonDown = 0;
